...
# $picardCMD SortSam I=$projPath/alignment/rmDuplicate/${histName}_bowtie2.sorted.rmDup.sam O=$projPath/alignment/rmDuplicate/${histName}_bowtie2.sorted.rmDup.sorted.sam SORT_ORDER=queryname

# ml SAMtools/1.10-foss-2016b
# mkdir -p $projPath/alignment/sam/fragmentLen

# ## Extract the 9th column from the alignment sam file which is the fragment length
# samtools view -F 0x04 $projPath/alignment/rmDuplicate/${histName}_bowtie2.sorted.rmDup.sorted.sam | awk -F'\t' 'function abs(x){return ((x < 0.0) ? -x : x)} {print abs($9)}' | sort | uniq -c | awk -v OFS="\t" '{print $2, $1/2}' >$projPath/alignment/sam/fragmentLen/${histName}_fragmentLen.txt

# ## 3. In preparation for SEACR peak calling
# echo "3. Data cleaning and format convertion"
# ml SAMtools/1.10-foss-2016b
# samtools view -bS -F 0x04 $projPath/alignment/rmDuplicate/${histName}_bowtie2.sorted.rmDup.sorted.sam >$projPath/alignment/bam/${histName}_bowtie2.mapped.bam

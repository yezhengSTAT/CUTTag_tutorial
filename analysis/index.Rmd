...
## 2.2. Quality Control using [FastQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) [Optional]
...
## 3.2. Filtering mapped reads by the mapping quality filtering [optinal]
...
### 6.1.2 Distribution of the peak length
...
### 6.1.4 FRagment proportion in Peaks regions (FRiPs).
...
frip = left_join(inPeakData, alignResult, by = c("Histone", "Replicate")) %>% mutate(frip = inPeakN/alignNum * 100)
...
### 7.2.1 Heatmap over transcripts
...
computeMatrix scale-regions -S $projPath/alignment/bigwig/${histName}_raw.bw \
              -R $projPath/peakCalling/SEACR/${histName}_seacr_control.peaks.stringent.bed \
              --skipZeros -o $projPath/peakCalling/SEACR/${histName}_SEACR.mat.gz -p $cores
